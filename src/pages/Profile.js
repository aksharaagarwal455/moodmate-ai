...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    // Fetch mood history from MySQL for the chart
  ...
  const chartData = {
    ...
    datasets:[{
      ...
      fill: false, borderColor:'#a78bfa', pointBackgroundColor:'#a78bfa',
      pointBorderColor:'#0d0d1a', pointBorderWidth:2, pointRadius:5, tension:0.4
    }]
  };
  const chartOpts = {
    ...
    scales:{
      ...
      y:{min:1,max:7,ticks:{stepSize:1,color:'rgba(240,238,255,0.25)',font:{family:'Nunito',size:11},
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .p-sidebar{width:230px;min-height:100vh;background:#10101e;border-right:1px solid rgba(255,255,255,0.05);display:flex;flex-direction:column;padding:28px 0;position:fixed;top:0;left:0;z-index:100;}
        .p-logo{font-family:'Syne',sans-serif;font-size:1.4rem;font-weight:800;background:linear-gradient(135deg,#a78bfa,#f472b6);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;padding:0 24px;margin-bottom:40px;}
        .p-nav{flex:1;padding:0 12px;display:flex;flex-direction:column;gap:2px;}
        .p-nav-label{font-size:0.62rem;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:rgba(240,238,255,0.2);padding:0 12px;margin:8px 0 6px;}
        .p-nav-btn{display:flex;align-items:center;padding:11px 14px;border-radius:10px;cursor:pointer;font-size:0.9rem;font-weight:600;color:rgba(240,238,255,0.4);border:none;background:none;font-family:'Nunito',sans-serif;width:100%;text-align:left;transition:all 0.15s;}
        .p-nav-btn:hover{background:rgba(255,255,255,0.05);color:rgba(240,238,255,0.8);transform:none !important;}
        .p-nav-btn.active{background:linear-gradient(135deg,rgba(124,58,237,0.2),rgba(236,72,153,0.1));color:#c4b5fd;}
        .p-sidebar-bottom{padding:0 12px;border-top:1px solid rgba(255,255,255,0.05);padding-top:16px;margin-top:16px;}
        .p-user-row{display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:10px;}
        .p-avatar{width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg,#7c3aed,#ec4899);display:flex;align-items:center;justify-content:center;font-size:0.72rem;font-weight:800;color:white;flex-shrink:0;}
        .p-user-name{font-size:0.88rem;font-weight:600;color:rgba(240,238,255,0.6);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
        .p-signout{display:flex;align-items:center;padding:10px 14px;border-radius:10px;cursor:pointer;font-size:0.85rem;font-weight:600;color:rgba(240,238,255,0.25);border:none;background:none;font-family:'Nunito',sans-serif;width:100%;text-align:left;transition:all 0.15s;margin-top:2px;}
        .p-signout:hover{color:rgba(248,113,113,0.8);background:rgba(248,113,113,0.06);transform:none !important;}
        ...
        @media(max-width:900px){.p-sidebar{display:none;}.p-main{margin-left:0;padding:24px;}}
      `}</style>
      <div className="p-root">
        <aside className="p-sidebar">
          <div className="p-logo">MoodMate</div>
          <nav className="p-nav">
            <div className="p-nav-label">Main</div>
            <button className="p-nav-btn" onClick={()=>navigate('/dashboard')}>Discover</button>
            <button className="p-nav-btn" onClick={()=>navigate('/history')}>History</button>
            <button className="p-nav-btn active">Profile</button>
          </nav>
          <div className="p-sidebar-bottom">
            <div className="p-user-row">
              <div className="p-avatar">{initials}</div>
              <span className="p-user-name">{firstName}</span>
            </div>
            <button className="p-signout" onClick={()=>{localStorage.clear();navigate('/');}}>Sign out</button>
          </div>
        </aside>
        ...
        <main className="p-main">
          ...
          <div className="p-card">
            ...
            <form className="p-form" onSubmit={handleUpdate}>
              ...
              <button className="p-save" type="submit" disabled={loading}>
