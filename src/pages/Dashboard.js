...
export default function Dashboard({ fetchMusicByMood, tracks=[], mood='', loading=false, noResult=false, advice='', playlist=null }) {
  ...
  useEffect(() => { document.body.style.background = '#0d0d1a'; return () => { document.body.style.background=''; }; }, []);

  const MOOD_COLORS = { happy:'#fbbf24', sad:'#60a5fa', angry:'#f87171', surprised:'#a78bfa', disgusted:'#34d399', fearful:'#f472b6', neutral:'#94a3b8' };
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .db-nav-section-label {
          ...
          text-transform:uppercase; color:rgba(240,238,255,0.2);
        ...
        .db-nav-btn {
          ...
          font-size:0.9rem; font-weight:600; color:rgba(240,238,255,0.4);
          border:none; background:none; font-family:'Nunito',sans-serif;
          width:100%; text-align:left; transition:all 0.15s;
        }
        .db-nav-btn:hover { background:rgba(255,255,255,0.05); color:rgba(240,238,255,0.8); transform:none !important; }
        .db-nav-btn.active { background:linear-gradient(135deg,rgba(124,58,237,0.2),rgba(236,72,153,0.1)); color:#c4b5fd; }

        .db-sidebar-bottom { padding:0 12px; border-top:1px solid rgba(255,255,255,0.05); padding-top:16px; margin-top:16px; }

        .db-user-row { display:flex; align-items:center; gap:10px; padding:10px 12px; border-radius:10px; }
        ...
        .db-user-name { font-size:0.88rem; font-weight:600; color:rgba(240,238,255,0.6); overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }

        .db-signout {
          display:flex; align-items:center; padding:10px 14px; border-radius:10px;
          cursor:pointer; font-size:0.85rem; font-weight:600;
          color:rgba(240,238,255,0.25); border:none; background:none;
          font-family:'Nunito',sans-serif; width:100%; text-align:left;
          transition:all 0.15s; margin-top:2px;
        }
        .db-signout:hover { color:rgba(248,113,113,0.8); background:rgba(248,113,113,0.06); transform:none !important; }
        ...
        .db-advice {
          ...
          font-size:0.9rem; color:rgba(240,238,255,0.55);
        ...
        .db-track-link { font-size:0.75rem; color:#a78bfa; font-weight:600; text-decoration:none; letter-spacing:0.02em; }
        ...
        @media (max-width:900px) {
          .db-sidebar{display:none;} .db-main{margin-left:0;}
          .db-topbar,.db-content{padding:24px;}
      ...
      <div className="db-root">
        <aside className="db-sidebar">
          <div className="db-logo">MoodMate</div>
          <nav className="db-nav">
            <div className="db-nav-section-label">Main</div>
            <button className="db-nav-btn active" onClick={()=>navigate('/dashboard')}>Discover</button>
            <button className="db-nav-btn" onClick={()=>navigate('/history')}>History</button>
            <button className="db-nav-btn" onClick={()=>navigate('/profile')}>Profile</button>
          </nav>
          <div className="db-sidebar-bottom">
            <div className="db-user-row">
              <div className="db-avatar">{initials}</div>
              <span className="db-user-name">{firstName}</span>
            </div>
            <button className="db-signout" onClick={()=>{localStorage.clear();navigate('/');}}>Sign out</button>
          </div>
        </aside>
        ...
        <main className="db-main">
          ...
          <div className="db-content">
            ...
            <AnimatePresence>
              {mood && moodData && (
                <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:0.25}} className="db-mood-pill">
            ...
            <AnimatePresence>
              {advice && (
                <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:0.25,delay:0.1}} className="db-advice">
                  {advice}
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="db-tracks-grid">
                  {tracks.map((track, i) => (
                    <motion.div key={track.id||i} className="db-track-card"
                      initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:0.3,delay:i*0.045}}>
